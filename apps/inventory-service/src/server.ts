...
// 3. Import container and start event consumers BEFORE listening.
const { InventoryContainer } =
  await import("./container/inventory.container.js");
logger.info({ module: "server" }, "Starting inventory event consumers...");
await InventoryContainer.getInstance().start();

// 4. Import app.js after dependencies and consumers are ready.
...
await startServer({
  ...
  beforeShutdown: async () => {
    logger.info({ module: "server" }, "Stopping inventory event consumers...");
