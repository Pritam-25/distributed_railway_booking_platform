...
export const BackoffOptionsSchema = z
  .object({
    retries: z.number().int().positive().max(50).optional(),
    initialMs: z.number().int().positive().max(1000).optional(),
    maxMs: z.number().int().positive().max(30000).optional(),
  })
...
export async function withExponentialBackoff<T>(
  ...
  const optionsWithDefaults = {
    ...validated,
    retries: validated.retries ?? 5,
    initialMs: validated.initialMs ?? 100,
    maxMs: validated.maxMs ?? 10000,
  };
  ...
  while (true) {
    try {
      ...
      attempt++;
      if (attempt > optionsWithDefaults.retries) {
        throw error;
      }
      const delay = Math.min(
        optionsWithDefaults.maxMs!,
        optionsWithDefaults.initialMs! * Math.pow(2, attempt - 1),
      );
